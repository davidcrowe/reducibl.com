...
to prevent users from messing with each other, each demo session now gets its own lightweight tenant in firestore. that means isolated policy configs, rate limits, and audit logs. it's a bit more complex, but it ensures a clean and consistent experience for everyone.
